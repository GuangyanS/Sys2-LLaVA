...
from SwiLLaVA.llava.model.builder import load_pretrained_model as load_pretrained_model_vqa
from SwiLLaVA.llava.utils import disable_torch_init
from SwiLLaVA.llava.mm_utils import get_model_name_from_path, tokenizer_image_object_token, KeywordsStoppingCriteria

from ROILLaVA.llava.model.builder import load_pretrained_model as load_pretrained_model_roi
from ROILLaVA.llava.mm_utils import tokenizer_image_token, process_images
...
class VQA_LLM:
	def __init__(self, args):
		...
		model_name += 'llava'
		self.tokenizer, self.model, self.image_processor, self.context_len = load_pretrained_model_vqa(model_path, args.vqa_model_base, model_name)
		print('Loading Adapter...')
		self.model = PeftModel.from_pretrained(self.model, "/home/gs3260/gysun/projs/Sys2-LLaVA/SwiLLaVA/checkpoints/seal_vqa_7b", adapter_name="switch")
...
class ROI_LLM:
	def __init__(self, args):
		model_path = args.roi_model_path
		model_name = get_model_name_from_path(model_path)
		model_name += 'llava'
		model_base = None
		device_map = "auto"
		self.tokenizer, self.model, self.image_processor, self.context_len = load_pretrained_model_roi(model_path, model_base, model_name)
		self.conv_type = args.conv_type

	@torch.inference_mode()
	def roi_inference(self, image, question, temperature=0.2, top_p=None, num_beams=1, max_new_tokens=128, object_crops=None, images_long=None, objects_long=None):
		conv = conv_templates[self.conv_type].copy()
	...
	for test_type in ['direct_attributes', 'relative_position']:
		...
		for image_file in tqdm(image_files):
			...
			if len(missing_objects) > 0:
				outputs = roi_llm.roi_inference(image, question)
				try:
...
if __name__ == "__main__":
	...
	parser.add_argument("--vqa-model-path", type=str, default="/home/gs3260/gysun/projs/Sys2-LLaVA/SwiLLaVA/checkpoints/seal_vqa_7b")
	...
	parser.add_argument("--benchmark-folder", type=str, default="/home/gs3260/gysun/datasets/vstar_bench")
	parser.add_argument("--roi-model-path", type=str, default="Zuyan/llava-CoS-13B")
