...
from .multimodal_projector.builder import build_vision_projector, build_vision_projector_v2

from ROILLaVA.llava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            if hasattr(config, 'mm_vision_load_qformer') and config.mm_vision_load_qformer is True:
                self.mm_projector = build_vision_projector_v2(config)
            else:
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        # import pdb; pdb.set_trace()
        if self.config.mm_vision_tower == 'query_qformer':
            self.config.mm_vision_load_qformer = True
            self.mm_projector = build_vision_projector_v2(self.config)
        else:
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, attention_mask, past_key_values, labels, images, images_crop
    ):
